...
namespace SV22T1020553.Admin.Controllers
{
    ...
    public class EmployeeController : Controller
    {
        ...
        {
            var input = ApplicationContext.GetSessionData<PaginationSearch>(SEARCH_EMPLOYEE);
            if (input == null)
            {
        ...
        public async Task<IActionResult> ChangePassword(int id, string password, string confirmPassword)
        {
            ...
            // 1. Kiểm tra rỗng và khớp mật khẩu
            ...
            // 2. QUAN TRỌNG: Phải băm mật khẩu mới bằng MD5 trước khi lưu vào DB
            string passwordHash = CryptHelper.HashMD5(password);

            // 3. Lưu mật khẩu ĐÃ BĂM vào database
            bool success = await HRDataService.ChangeEmployeePasswordAsync(employee.Email, passwordHash);

            if (success)
            {
                // Có thể thêm thông báo thành công vào TempData
                TempData["Message"] = "Đã đổi mật khẩu thành công";
                return RedirectToAction("Index");
            }

            ModelState.AddModelError("Error", "Không thể cập nhật mật khẩu");
